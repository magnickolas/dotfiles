...
import XMonad (
    ...
    Layout,
    X,
    XConfig (XConfig, borderWidth, focusFollowsMouse, keys, layoutHook, logHook, manageHook, modMask, startupHook, terminal, workspaces),
    className,
    handleEventHook,
    io,
    kill,
...
myKKeys XConfig{modMask = modMask} =
    M.fromList $
